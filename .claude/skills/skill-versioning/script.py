...
Gerencia versionamento do projeto.
...
def write_version(major: int, deploy_count: int, year: int) -> str:
    """Escreve a nova versao no arquivo VERSION e package.json."""
    ...
    if PACKAGE_JSON.exists():
        try:
            ...
            package_data['version'] = version_string

            with open(PACKAGE_JSON, 'w') as f:
                json.dump(package_data, f, indent=2)
                f.write('\n')

            print(f"package.json atualizado: {old_version} -> {version_string}")
        except Exception as e:
            print(f"Aviso: Erro ao atualizar package.json: {e}")
...
def increment_major() -> str:
    ...
    return write_version(major + 1, 1, CURRENT_YEAR)
...
def set_version(version_str: str) -> str:
    ...
    return write_version(int(match.group(1)), int(match.group(2)), int(match.group(3)))
...
def log_deploy(old_version: str, new_version: str):
    ...
    if CHANGELOG_FILE.exists():
        ...
        new_content = f"# Changelog\n\nTodas as mudancas notaveis do projeto.\n{log_entry}"
...
def main():
    parser = argparse.ArgumentParser(
        description='Gerenciador de Versionamento',
