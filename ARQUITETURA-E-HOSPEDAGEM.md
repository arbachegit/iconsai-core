...
| **2** | **brasil-data-hub** (frontend) | `geaaxpctnixwsppgttsm` | Frontend conectado (.env) | Vite app |
...
### 2️⃣ PROJETO SUPABASE: `brasil-data-hub` (geaaxpctnixwsppgttsm)

**Localização:** `.env`

```env
VITE_SUPABASE_PROJECT_ID="geaaxpctnixwsppgttsm"
VITE_SUPABASE_URL="https://geaaxpctnixwsppgttsm.supabase.co"
```
...
- Mas o frontend está tentando chamar no projeto `geaaxpctnixwsppgttsm`
...
┌─────────────────────────────────────────────────────┐
│ Frontend (.env)                                     │
│ └─ Conecta em: geaaxpctnixwsppgttsm                │
...
**Consequência:**
- ❌ Frontend chama: `https://geaaxpctnixwsppgttsm.supabase.co/functions/v1/pwacity-openai`
...
1. Atualizar `.github/workflows/deploy-supabase.yml`:
   ```yaml
   SUPABASE_PROJECT_ID: geaaxpctnixwsppgttsm  # ao invés de gmflpmcepempcygdrayv
   ```

2. Atualizar `supabase/config.toml`:
   ```toml
   project_id = "geaaxpctnixwsppgttsm"  # ao invés de gmflpmcepempcygdrayv
   ```

3. Re-deploy manual das funções:
   ```bash
   supabase functions deploy pwacity-openai --project-ref geaaxpctnixwsppgttsm
   supabase functions deploy pwacity-gemini --project-ref geaaxpctnixwsppgttsm
   ```
...
2. Migrar dados de `geaaxpctnixwsppgttsm` para `gmflpmcepempcygdrayv`
...
1. **CORRIGIR DEPLOY DAS EDGE FUNCTIONS:**
   ...
   SUPABASE_PROJECT_ID: geaaxpctnixwsppgttsm

   # Atualizar supabase/config.toml
   project_id = "geaaxpctnixwsppgttsm"

   # Re-deploy manual
   supabase functions deploy --project-ref geaaxpctnixwsppgttsm
   ```
...
3. **DEPRECAR PROJETO `gmflpmcepempcygdrayv`:**
   - Migrar todas as Edge Functions para `geaaxpctnixwsppgttsm`
...
2. **Sobre os 3 projetos Supabase:**
   - ✅ `geaaxpctnixwsppgttsm` = Frontend + Edge Functions (CONSOLIDAR AQUI)
