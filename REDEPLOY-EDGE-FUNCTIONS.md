...
|---------|---------|--------|
| `.github/workflows/deploy-supabase.yml` | `gmflpmcepempcygdrayv` → `geaaxpctnixwsppgttsm` | ✅ Commitado |
| `supabase/config.toml` | `gmflpmcepempcygdrayv` → `geaaxpctnixwsppgttsm` | ✅ Commitado |
...
Você precisa fazer um **re-deploy manual** para movê-las para o projeto correto (`geaaxpctnixwsppgttsm`).
...
supabase link --project-ref geaaxpctnixwsppgttsm
```
...
```bash
supabase functions list --project-ref geaaxpctnixwsppgttsm
```
...
```bash
curl -X POST "https://geaaxpctnixwsppgttsm.supabase.co/functions/v1/pwacity-openai" \
...
**Se der erro:**
- Verifique os logs: https://supabase.com/dashboard/project/geaaxpctnixwsppgttsm/logs/edge-functions
- Execute o re-deploy
...
**Causa:** Função ainda não foi deployada no projeto `geaaxpctnixwsppgttsm`
...
**Verificar:**
```
https://supabase.com/dashboard/project/geaaxpctnixwsppgttsm/settings/functions
```
