...
export const DataFlowDiagram: React.FC = () => {
  ...
  const svgDimensions = { width: 1600, height: 700 };
  ...
  const getPosition = useCallback((id: string) => {
    return (
      sourcePositions.find(p => p.id === id) ||
      indicatorPositions.find(p => p.id === id) ||
      knowledgePositions.find(p => p.id === id) ||
      deliveryPositions.find(p => p.id === id)
    );
    ...
    <div ref={containerRef} className="w-full space-y-4">
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h2 className="text-xl font-bold text-foreground">Gov System Icons AI</h2>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-card/50 border border-border rounded-xl p-4">
        <FilterControl
          ...
          onToggle={toggleFilter as (domain: DomainType) => void}
          onReset={resetFilters}
        />
        <StepByStepControl
          currentStep={state.currentStep}
          isAnimating={state.isAnimating}
          onStart={startStepByStep}
          onNext={nextStep}
          onStop={stopStepByStep}
        />
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-x-auto">
        <svg
          ...
          onClick={() => state.selectedElement && setSelectedElement(null)}
        >
          <defs>
          ...
          {visibleConnections.map(conn => {
            ...
            const isActive = 
              state.hoveredElement?.id === conn.from ||
              state.hoveredElement?.id === conn.to ||
              connectedElements.has(conn.from) ||
              connectedElements.has(conn.to);

            const fromZone = conn.from.startsWith("ind-") ? "indicators" : 
                             conn.from.startsWith("know-") ? "knowledge" : "sources";
            const toZone = conn.to.startsWith("ind-") ? "indicators" : 
                          conn.to.startsWith("know-") ? "knowledge" : 
                          conn.to.startsWith("painel-") ? "delivery" : "content";
