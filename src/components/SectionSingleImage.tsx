...
const markCreditsExhausted = () => {
  localStorage.setItem(CREDITS_EXHAUSTED_KEY, JSON.stringify({
    timestamp: Date.now()
  }));
};
...
export const SectionSingleImage = ({
  sectionId,
  priority = false
  ...
  useEffect(() => {
    ...
    const generateImage = async () => {
      ...
      const {
        data: existingImage
      } = await supabase.from('generated_images').select('image_url').eq('section_id', sectionId).eq('prompt_key', promptKey).maybeSingle();
  ...
  return <div ref={ref} className="w-full h-full">
      {usesFallback && <div className="mb-2 text-xs text-muted-foreground text-center opacity-75">
          <p>Imagem temporária (créditos esgotados)</p>
        </div>}
      <div className="w-full h-full rounded-lg overflow-hidden bg-muted/10">
        {image ? <OptimizedImage src={image} alt={`${sectionId} - Ilustração`} priority={priority} aspectRatio="square" /> : <ImagePlaceholder className="w-full h-full" />}
      </div>
    </div>;
};